"""Schema-driven ODS-to-JSON conversion for portfolio worksheets."""
...
def load_schema(path: Path) -> dict:
    """Load one JSON conversion schema."""
...
def load_sheet(path: Path, sheet_name: str) -> ElementTree.Element:
    """Load one worksheet from an ODS archive."""
...
def element_text(element: ElementTree.Element) -> str:
    """Extract ODF text while honoring explicit whitespace."""
...
def nonempty_rows(table: ElementTree.Element, separator: str) -> list[tuple[int, list[str]]]:
    """Return logical row numbers and nonempty row values."""
    result = []
    logical_row = 1
...
def extend_headers(headers: list[str], conversion: dict) -> list[str]:
    """Append declared names for otherwise unnamed source columns."""
    result = list(headers)
    unnamed = conversion.get("unnamedColumns", {}).items()
    for column, name in sorted(unnamed, key=lambda item: int(item[0])):
        if int(column) != len(result) + 1:
...
def source_records(rows: list, conversion: dict) -> tuple[list[str], list[tuple[int, dict]]]:
    """Map logical ODS rows to dictionaries keyed by headers."""
    header_number = conversion["rows"]["header"]
    values = next((value for number, value in rows if number == header_number), None)
    if not values or len(values) != len(set(values)):
        raise ValueError("worksheet has missing or duplicate headers")
    headers = extend_headers(values, conversion)
    records = []
    for row_number, values in rows:
        if row_number < conversion["rows"]["dataStart"]:
            continue
        if any(values[len(headers) :]):
            raise ValueError(f"row {row_number} contains data beyond the headers")
...
def record_properties(schema: dict) -> dict:
    """Resolve the record definition referenced by the records array."""
    reference = schema["properties"]["records"]["items"]["$ref"]
    prefix = "#/$defs/"
    if not reference.startswith(prefix):
        raise ValueError("record schema must use a local $defs reference")
    definition = schema["$defs"][reference.removeprefix(prefix)]
    properties = definition["properties"]
    if set(definition["required"]) != set(properties):
        raise ValueError("record properties and required fields differ")
    return properties


def validate_mapping(headers: list[str], schema: dict) -> dict:
    """Validate worksheet columns and transform declarations."""
    properties = record_properties(schema)
    mapped = {
    ...
    if mapped != set(headers):
        ...
        raise ValueError(f"schema and worksheet columns differ: {difference}")
    configured = schema["x-conversion"]["transforms"]
    unknown = {rules["x-transform"] for rules in properties.values()} - set(configured)
    if unknown:
        raise ValueError(f"schema has undefined transforms: {', '.join(sorted(unknown))}")
    return properties
...
def csv_string_list(value: str) -> list[str]:
    """Parse one CSV-encoded list and remove duplicates."""
    if not value:
        return []
    parsed = next(csv.reader([value], strict=True, skipinitialspace=True))
    items = (item.strip() for item in parsed)
    return list(dict.fromkeys(item for item in items if item))


def reference_urls(value: str) -> list[str]:
    """Extract HTTP URLs and normalize bare DOIs in source order."""
...
def number_or_range(value: str) -> int | float | dict | None:
    """Parse an optional number or inclusive hyphen range."""
    if not value:
        return None
...
def integer_or_range(value: str) -> int | dict | None:
    """Parse an optional TRL integer or inclusive slash range."""
    if not value:
        return None
...
def integer_or_range_or_na(value: str) -> int | dict | None:
    """Parse a TRL while treating an NA marker as missing."""
    return None if value.strip().upper() == "NA" else integer_or_range(value)
...
TRANSFORMS = {
    ...
    "integer-or-slash-range-or-na": integer_or_range_or_na,
}
...
def build_document(schema: dict, input_path: Path, records: list[dict]) -> dict:
    """Build the schema-shaped output document."""
...
def serialize_json(document: dict, settings: dict) -> bytes:
    """Serialize one document as deterministic UTF-8 JSON."""
...
def convert_schema(schema_path: Path) -> tuple[Path, int]:
    """Convert the worksheet configured by one schema."""
    ...
    data = serialize_json(document, conversion["serialization"])
    output_path.parent.mkdir(parents=True, exist_ok=True)
    write_atomic(output_path, data)
    return output_path, len(records)


def run_conversion(label: str, schema_path: Path) -> int:
    """Run one configured conversion with concise diagnostics."""
    try:
        output_path, record_count = convert_schema(schema_path)
    except (OSError, KeyError, ValueError, ElementTree.ParseError, zipfile.BadZipFile) as error:
        print(f"error: {error}", file=sys.stderr)
        return 1
    print(f"{label}: {record_count} records -> {output_path}")
    return 0
